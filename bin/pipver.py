...
@click.option('--extension', default=True, help='Increment the version Extension.')
...
@click.option('--dont-tag', is_flag=True, default=False, help='Skip tagging the commit with the generated version.')
@click.option('--git-push', is_flag=True, default=False, help='Also perform a `git push` after tagging.')
@click.option('--yes', is_flag=True, default=False, help='Always answer Yes when prompted with a Y/n question.')
def main(filepath, version, major, minor, patch, extension, build, from_git, dont_tag, git_push, yes):
    """
    main
    """
    # init git
    ...
    if dont_tag:
        ...
        if git_push:
            print('%s Skipping `git push` since we aren\'t tagging the commit...')
    else:
        ...
        if git_push:
            _git.git.push('-u origin %s' % _git.active_branch.name)
